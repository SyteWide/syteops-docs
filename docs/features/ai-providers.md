...
**Reading the balance.** The same fal key you paste above can generate images fine while still being refused when SyteOps tries to check your account balance — fal only allows balance reads from an **Admin** key on the account that pays the bill, not from a **Team** key. If the balance explains that it can't read your account, put an Admin key from the client's fal team (fal.ai/dashboard/keys) in the **fal key** field above — fal has no way to scope a key any narrower than that, so the same key that generates images also reads the balance once it's an Admin key.

**fal billing key (deprecated).** An older, separate key some sites used to work around this before an Admin key was known to cover both jobs. It no longer unblocks anything a fal key above doesn't already fix, so it appears on this tile only for a site that already has one stored — clear it there once the fal key above is an Admin key. It is encrypted, masked, and excluded from exports the same way every other key here is.
...
At the foot of the same card sits a read-only **Image generation & modify (SyteHero)** row. It reports whether SyteHero is installed and active, whether its image-AI key is configured, and — once you've checked it from the balance popup or the Review Portal's own image tools — its cached credit balance. When available, the row also names **this month's image-generation spend** — something like "This month: $12.50 · 3 generations (fal list price)" — SyteHero's own local record, not a live vendor read, so it appears with no extra click. If any of that spend is an estimate rather than an exact figure, the row says "fal prices & estimates" instead of "fal list price". Once SyteHero confirms a picture's ACTUAL billed charge with fal (rather than its own list-price estimate), a fully-confirmed month instead says "fal billed", and a month with some confirmed and some not says "fal billed + list prices"; if a generation went through with no price on record at all, the row says so ("N not priced") rather than understating the total. When SyteHero can also read your fal TEAM's total usage, the row appends "fal team total this month: $X" — the whole team's spend across every site sharing that key, alongside this site's own figure. When SyteHero is active, the row links straight to **Open SyteHero settings**; when it isn't, the row simply says image AI needs SyteHero. This row has no controls of its own — it powers picture remix, generate-new-picture, and the listing-image watermark repair. Where its key and models are configured depends on one setting: SyteHero's own settings page by default, or the **SyteHero image AI (fal)** tile on **System / API** when you have turned management on there, in which case the row says so and links straight at the tile. If a balance has never been read successfully because your fal key can't check it, the row shows the same Admin-key hint the balance popup shows, instead of staying blank.
...
**SyteHero's row also shows this month's spend.** Below its balance figure, a line reads "This month: $X · N generations (fal list price)" — the total spent generating and remixing pictures on this site so far this month, from SyteHero's own local record. Click **Previous months** to expand up to five more months. If some of a month's spend is an estimate rather than an exact figure, the line says "fal prices & estimates" instead; once fal has confirmed the actual charge for every priced generation that month it says "fal billed" instead, or "fal billed + list prices" when only some are confirmed; a month where nothing was priced yet reads "price unknown" rather than a misleading $0.00. This line appears wherever the balance figure does — the branding modal and the **Image generation & modify (SyteHero)** row above.
...
- Your fal key can generate images but is a **Team** key, and fal only allows balance reads from an **Admin** key on the account that pays the bill. Put an Admin key from the client's fal team in the **fal key** field on the SyteHero image AI (fal) tile — see [SyteHero image AI (fal)](#sytehero-image-ai-fal) above; fal has no way to scope a key any narrower than that, so nothing else about image generation changes. If this site already has a **fal billing key** stored from before, clear it there too — it's deprecated and no longer needed.
