...
The email comes from your site, uses your site's logo, and includes a summary of the article title and source keyword. Two buttons sit directly under that summary, above the article photo: **Edit/View this article**, which opens this particular draft, and a second button that opens your own private review page — labeled **My Portal** if you're a team member with a linked WordPress account, or **See all my reviews** otherwise. On a phone the two buttons stack one above the other so both stay readable and tappable.
...
Once you are signed in, the portal shows the full draft. The header brand (logo and company name) opens the WordPress Dashboard in a new tab when you click it. In the article editor, the top bar uses two rows — logo and status pill on the first row (status on the far right), nav pills under the logo and **Signed in as** on the right of the second — so logos and names are not squeezed on narrow screens. **My Portal** in that bar opens your own private page without leaving the portal session. An account with no linked team-member record sees **See all my reviews** there instead, which opens the review queue.
...
The toolbar and the preview beneath it are framed in one color, with a small label inside the toolbar, so you always know what you are looking at: green **Live** means visitors can already see this page exactly as previewed, amber **Draft preview** or **Scheduled** means only reviewers can see it, and gray **Private** covers anything else. Switching between pages in a bundle, or publishing or unpublishing an article, updates the frame and label to match.
...
Click a page element in the preview to leave a note. The click opens a note dialog instead of following the page. Drag across words first if you want the note to quote that phrase; a click with nothing selected still comments on the whole element. Hold **Cmd** (Mac) or **Ctrl** (Windows) and click a picture to open the same image tools as Content Review and swap it into the page; use **Undo picture swap** in the toolbar to reverse the last swap. Press Escape to drop the highlight and close the note. Clicking an image shows that it is an image, its name, and a small thumbnail. A pin on the preview and its Feedback row share the same color. If that element has moved, the row is marked Moved rather than guessed. Handling or deleting a note in Feedback updates the pin on the preview at the same time — you do not need to refresh.

You can also send **one review link for several pages or posts**. Under **Content Pipelines → Review Portal → Page Reviews**, name the bundle, add an optional note for reviewers (line breaks are kept in the email), add the items in the order they should be walked (click the add box to see every eligible page and post, then type to narrow the list), pick the reviewers, then **Save bundle**. Each person you select sees those pages on My Portal, including pages that are already published. The bundle list names each page and links it to that page's review. **Send bundle** emails those reviewers in the same layout as the other review messages: a short heading, that note labeled with the sender's first and last name, then each page's name in one card, and every one of those links opens that page's own review; **See all my reviews** on that mail opens your private review page filtered to the bundle, on its **Pages** view. On your review page, bundled items are grouped under each bundle's name. Click a page in the list to select it, then **Send page** emails that one page or post as a page review without saving it into a bundle. **Copy link** copies the same walk link without emailing anyone. The reviewer opens a single link and uses the strip above the preview to move to the next or previous item. Comments stay on the item they are looking at.

A copied link is for looking at the site and leaving notes. The people on it can leave notes, reply to them and delete their own, but they cannot edit or publish anything — see [Commenting on someone else's review](#commenting-on-someone-elses-review). **Send bundle** is what gives the listed reviewers their usual editing rights on those items. The pages still show on each selected person's My Portal as soon as the bundle is saved.

**AI agent prompts.** On a page or appearance review, a card in the sidebar carries the "AI agent prompts" **Admins** badge. It turns the notes on this page into one copyable prompt for a coding agent to implement — each selected note becomes a task naming where it is on the page, what kind of change it is, the client's words verbatim, and how to check the change is done. Tick the notes to include (unresolved and updated notes are ticked for you), adjust the task type if needed, then **Generate** and **Copy** the result. **Copy** is always there beside the prompt, grayed out until there is something to copy; the last prompt you generated stays in the box (with a note of when and by whom) so you can come back and copy it again without regenerating. It only appears on page and appearance reviews — a plain article review has no pinned page for it to describe — and it is hidden by default: grant it under [Editing permissions](#editing-permissions) by setting the **AI agent prompts** area to Allow for a role or a specific person. Generating needs the **Agent prompts** AI area configured under [AI Models](#ai-models); until then the button is disabled with a note explaining why.
...
When content AI is set up for your site, the same bar also has **Remix with AI** (it reads **AI**). Select part of the article first — a sentence, a paragraph, a heading, a list, or a table — and the dialog offers **Selection** (checked by default) or **Whole article**; with nothing selected, or the cursor just blinking somewhere, only the whole article can be rewritten. Selecting only part of a paragraph, a list item, or a table cell still rewrites the whole paragraph, list, or table it sits in — a remix never leaves half a list or half a table behind. The dialog names the provider and model that will run the rewrite. The new words stay in the editor until you press **Save**. **Undo remix** puts the last body (or the part that was rewritten) back; **Redo remix** brings that remix back after an undo. The button is hidden on pages, appearance-only reviews, comment-only access, and for anyone whose **Remix text with AI** permission is Hide.

A very long article is refused before anything is sent, with the character limit named so you know why. If a rewrite would have dropped a picture, a link, a shortcode, or a whole table, it is refused instead of applied, and the message says what would have gone missing — checked against just the selected piece when you chose **Selection**, so a picture elsewhere in the article never blocks a small text fix. If your selection no longer matches the article by the time the rewrite comes back — because it was edited in another tab, say — you are told to select it again rather than having the wrong text replaced. **Revert to pre-remix** appears once you have remixed at least once and puts the article back exactly as it read right before that remix — including after you reload the page, unlike Undo remix, which only remembers what happened in the current browser tab.

The dialog also offers a row of instruction chips — **Shorter**, **Longer**, **Simpler**, **More formal**, **More conversational**, **Fix grammar only** — that fill in the instruction box with a ready-made sentence you can still edit before pressing **Remix**. Your administrator chooses which of these chips appear (see [Remix presets](content-pipelines.md#remix-presets)); if none are turned on, the row is simply not there and you can still type your own instruction. Every remix you run is kept in a short list under the dialog (time, whether it covered the selection or the whole article, and what you asked for), so you can compare a few attempts and bring an earlier one back with **Apply**, or **Discard** one you don't want to keep. Up to six recent attempts are kept per article; the oldest drops off once you pass that.
...
**Unused pictures** sits in the same sidebar, just after Featured Image. It lists files attached to this article that the body and featured image are not using. Deleting one of those files deletes the unused file, not a picture still showing in the article. The card follows the **Unused pictures** permission: Allow shows it, Hide removes it. You still need permission to upload files, and the card only ever offers files you have permission to delete — a WordPress role that can upload media but not delete its own posts (a bare Subscriber, for example) will see the card empty even with the permission set to Allow. If that happens, ask your site administrator to grant the delete-posts capability, or use the Author role.
...
**Reusing a picture from your fal library.** When SyteHero is connected with a team key, a **From library** button sits beside **Choose from library**. It opens a browser of pictures already generated for this client — on any of their sites — so you can reuse one instead of paying to generate it again. Filter by collection, or search when no collection is selected (search and the collection filter cannot be combined, the same way SyteHero's own library browser works); scroll for more with **Load more**; click a picture to see it larger, then press **Use**. The picture is copied into this site's media library the same way a **Choose from library** pick is applied — picking the same picture twice reuses the same copy rather than adding a second one. Video items in the library cannot be used here; only images. If the library cannot be reached — no key connected, the fal account does not have this feature, or fal is briefly unavailable — the message says why. **From library** appears only where **Choose from library** does, under the same permissions.

**Choosing where a new picture is filed.** Once you have opened **From library** at least once for this article, a **File into** dropdown appears in **Modify with AI**, listing the client's collections. Leave it on **Site default** to file a fresh generation the same way it always has been, or pick a specific collection to file that one generation somewhere else — the pick resets to **Site default** each time you open a different picture. It appears only once collections are known, and only where **From library** itself does.
...
**Create an original picture inspired by this one.** A checkbox, right below **Keep text and logos exactly as they are**, offers a different kind of generate: instead of editing the picture you started from, the AI writes a description of its style, mood, colors and people, then paints a brand new picture from that description alone — the picture's own pixels are never sent to the image generator. It ticks itself the moment you attach a reference picture, and also starts ticked when you open a picture that already has a reference attached through **Attach reference image** on **Notes on This Image** (your site can turn this default off either way), and unticking it for one generation sticks — including across attaching or changing a Notes reference — until you close and reopen the panel. While it is ticked, Strength, Preserve and Model photos are not used — they edit the picture you started from, and this generates a new one instead — and the Model list switches to the same text-to-image models **Generate New Image from Text** offers. A version made this way is badged **Original · inspired by &lt;file name&gt;** in the grid of versions. This creates a new picture from a written description rather than editing the one you started from. It lowers the chance of reproducing someone else's picture, but it is not legal advice: make sure you have the right to use any picture you start from.
...
Your site administrator sets the defaults under **Content Pipelines → Review Portal → AI Models → Image generation**; each Generate can override them. The **Create an original picture by default** checkbox — whether attaching a reference picture ticks **Create an original picture inspired by this one** for you — lives in the same **Review Portal → Modify images with AI** settings, right after **Preserve detail**.
...
Click it, choose who you want us to notify, optionally type a message, and press **Send**. The dialog lists the notes that person has not been sent yet — open notes, replies, and notes already handled that they were never told about. Every box starts checked; uncheck any you do not want in this email. That notes-list hint appears after you pick someone. **Check all** and **Uncheck all** above the list save you from clicking through them one at a time. If you uncheck every note and have not typed a message, **Send** disables with a reminder to pick at least one note or add a message — checking a box or starting to type clears it right away.
...
**A pass over every photo, two ways to reach it.** Generating a meta description, analyzing answer readiness or refreshing categories and tags each end by offering to run SEO on the article's photos too. The **Review Photo AI** button in the SEO panel opens the same picker directly, any time, without generating a meta description first. Either way you get a list of every picture with a checkbox — ticked in advance only where there is no alt text at all, including where the alt box just holds the file's own name, which is not a description of anything — plus **Check all** and **Uncheck all** links above the list. A picture you leave unticked is not touched. Each ticked picture is one AI call, and the count is shown before you start.

**Only pictures actually shown on the article are offered.** A picture attached to the post but not shown anywhere in it — not in the body, not set as the featured image — has nowhere for a description to be applied, so the list leaves it out and names how many it left out just above the count: *"N picture(s) attached to this article but not shown in it were skipped."* If nothing on the article can be described, the **Review Photo AI** button tells you so instead of opening an empty list; the automatic offer after a meta description simply says nothing, the way it already does when there are no pictures at all.
...
- On a **phone**, the month grid becomes a compact overview — each day with something scheduled is marked with a dot — and a **schedule list** appears below it, showing every post for that month with its time, title and status, plus your unscheduled drafts. Unscheduled drafts page five at a time, with the same 5/10/25/100 chooser as the review lists. That list is where you read and tap; the date picker opens from the bottom of the screen.
...
- **Bundle items are grouped.** Pages and posts in a bundle you review, including ones already published, are listed together under that bundle's name, in the order the bundles were created, with the name linking back to the bundle. Anything not part of a bundle is listed on its own beneath the groups, exactly as before.
- **The newest items first.** Each list shows five items at a time, with a chooser for 5, 10, 25, or 100. **Next** loads the next series from the server through the authorized catalog, so older authorized items are reachable. If the site has a very large backlog, a safety scan may still show a note reading *"Older items may not be listed."* When you see that note, some older articles may be waiting on you even though they aren't shown — open them from their original review email, or ask your site administrator.
- **A page-review bundle uses the same queue.** **My Portal** from a bundle opens this same page filtered to that bundle's remaining items, including pages and posts that are already published, with the bundle name under **Your Reviews**. If the bundle is missing or you are not listed on it, the page stays empty — it does not fall back to every draft you can review. A bundle list pages the same way, so older remaining items in that bundle are still reachable.
- **Older "See all my reviews" links open your private review page.** If you're a team member with a linked WordPress account, a **See all my reviews** link from any email you already have — and **My Portal** at the top of every review screen — now takes you to [your own private review page](#each-persons-private-review-page), still filtered to the bundle when the link came from one. If you aren't signed in, you sign in first and then land there. Opening someone else's private link while signed in as yourself is refused, with an option to switch accounts.
...
Filters apply everywhere on the page — **Awaiting Your Review**, **Watching**, **Coming Up**, **Notes & Replies for You**, and **Published Content** — and a section that no longer applies disappears rather than showing an empty list. The queue opens in **Posts | Pages** (two columns) unless you pick Posts, Pages, or All types. Choosing **Live** for status, for example, hides **Awaiting Your Review** entirely and shows only your live articles. Filtered lists still show five at a time, with the same 5/10/25/100 chooser, and **Next** loads the next series from the authorized catalog. If a safety scan hits its ceiling, the page can still show *"Older items may not be listed."* A **Clear filters** link appears once you've narrowed the list, taking you back to Posts | Pages. Filters are carried in the page's own link, so bookmarking or sharing a filtered view keeps the filter active; if you're using the older "See all my reviews" link, filters still keep your bundle scope.

### Watching

If you're copied as a **Default CC reviewer** on anything, those drafts and scheduled articles appear beneath your **Awaiting Your Review** list, under **Watching** — as long as you don't already have full reviewer access to them (those stay up in **Awaiting Your Review** instead, so nothing is listed twice). Rows open the same way as your other queue rows, and the section simply doesn't appear if you aren't watching anything. It pages the same way as the rest of the queue — five at a time, with the 5/10/25/100 chooser, and **Next** through the authorized catalog. When a safety scan may have missed older items it says *"Older ones may not be listed."* rather than quietly leaving them out — including when nothing at all could be shown within that scan, so the section appears with just that note instead of disappearing. If everything you were watching turned out to be scheduled, the note points you at **Coming Up** where those items are listed, rather than telling you none were found.
...
Beneath the drafts awaiting you, **Published Content** lists any **published** posts and pages you're assigned to. That heading matches **Your Reviews** at the top of the page. Open **Live Posts and Pages You Can Edit** when you want them — Posts and Pages inside that list, and under **Awaiting Your Review**, are labeled in capitals. If you have none, the section doesn't appear at all.
...
- **Published or scheduled** *(on by default)* — tells the author, the reviewers and anyone watching the article when a draft is scheduled, and again when it goes live. The person who published or scheduled it is not emailed — they already saw it happen. If the article was published with private visibility, the email says so and links to it as a private post, rather than describing it as live on the site.
  ...
  The **now live** email goes out however the article went live: from the portal, the content calendar, the WordPress editor, or on its own at its scheduled time (then everyone involved is told). Each go-live sends it once. Saving an article that is already live, or changing a live article's published date, sends nothing; taking it back to draft and publishing it again sends it again. It is only sent for articles that came through review — delivered by a content source, sent to reviewers, or part of a bundle. Posts you write and publish yourself, and articles from a source set to publish as soon as they arrive, stay quiet. While Content Pipelines is in **Setup** mode, articles that go live outside the portal send nothing. The email's main button opens the live article. If you're a team member with a linked WordPress account, a second button labeled **My Portal** opens your own private review page; if you aren't linked, that button reads **See all my reviews** and opens the shared review queue instead. If your mail server refuses the notice outright and nobody receives it, the notice is retried once a few minutes later; if that also fails, republishing the article sends it.
...
Every AI choice the portal makes is here — one row per feature, named by what it powers, plus a readiness pill (**Ready**, **Needs key**, or **Not configured**):

- **Article writing & remix** (Content) — rewrites, meta descriptions and taxonomy suggestions inside the portal.
- **Answer-engine analysis** (GEO) — the answer-engine analysis in the GEO panel.
- **Ingest field mapping** (Ingest) — proposes how an inbound content-source payload maps to your post fields.
- **Social posts** (Social) — composes the post text for **Post to Social**. Its provider/model save with this card; the Social Publishing card further down keeps its own profiles, destinations and enable toggle.
- **Image alt & captions** (Image SEO, carries a **Vision** badge) — what **Generate SEO with AI** uses for alt text and captions. This is not SyteHero and not Modify with AI. Needs a model that accepts image input, so any provider that cannot take one is shown unselectable — see [AI Providers](./ai-providers.md) for which those are.
- **Agent prompts** — turns page-review notes into an implementation prompt for a coding agent.

Two more rows are status-only — a pill, a "Powers: …" line and a link, no controls here:

- **Lead scoring** — shown only when the Leads module is active; links to its own page.
- **LinkCentral** — shown only when the integration is on, as three rows (Cross-Link, Keyword, Context AI); links to its Enrichment Settings on **System / API**.
...
| **AI agent prompts** | Allow or Hide (the AI agent prompts card on page and appearance reviews). Defaults to Hide — administrators see it either way. |
...
The last row of that summary is **Your private link** — the link to this page, with a **Copy** button beside it. Open **Your details** to see it. There is no **Regenerate** button here: replacing a link stays with site administrators on the **Users** tab.
