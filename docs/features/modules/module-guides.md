...
Creates encrypted `.sytepkg` files from modules and FlowMattic workflow templates. Package Builder turns your internal tools and workflows into distributable, encrypted packages that protect your source code while making deployment repeatable.

### When you need it

Enable Package Builder if you are:
- An **agency** distributing internal modules or workflow templates across client sites
- A **developer** building modules or workflow packages for distribution to others
- Anyone who needs to package FlowMattic workflows for repeatable deployment

### Getting started

1. Upload the Package Builder module package via **Admin > Modules > Upload**
2. Toggle the module **ON** in the modules list
3. Configure builder mode settings
4. Select a module or FlowMattic workflows to package
5. Package Builder encrypts and signs the contents, producing a `.sytepkg` file

### Key behaviors and settings

- **Encryption and signing** -- All packages are encrypted so source code cannot be read without a valid license. Digital signatures ensure tampering is detected on install.
- **Module packages** -- Package a full installable module as a single `.sytepkg` file.
- **Workflow template packages** -- Package FlowMattic workflow definitions and their associated variables for deployment to other sites.
- **Builder mode configuration** -- Required before creating packages. Configure through the module's settings.

See [Encrypted Packages](packages) for details on the `.sytepkg` format, installation, and troubleshooting.
